...
SOURCES_PATH="$SOURCES_PATH_DEFAULT/therriscoin"
BACKUP_PATH_DEFAULT=$HOME/therrisbackup
BACKUP_PATH=$BACKUP_PATH_DEFAULT
REPO="https://github.com/therriscoin/therriscoin"
BRANCH="master"
SWAP_FILE="$HOME/therriscoin-swap"
...
if [ -d "../.git" ]; then
	...
	if [ -f "$SOURCES_PATH/bin/therrisd" ]; then
		rm -f $SOURCES_PATH/bin/therrisd
		echo -e "${BLUE}Removed old binary in $SOURCES_PATH/bin/${NC}"
	fi
	if [ -f "../bin/therrisd" ]; then
      rm ../bin/therrisd
	...
	SOURCES_PATH="${SOURCES_PATH:-$SOURCES_PATH_DEFAULT}/therriscoin"
...
sudo killall -9 therrisd
if [ -f "/usr/local/bin/therrisd" ]; then
  sudo rm -f /usr/local/bin/therrisd
  echo -e "${BLUE}Removed therrisd in /usr/local/bin/ - To be replaced with new version${NC}"
fi
...
if [ ! -f $SOURCES_PATH/bin/therrisd ]; then
	echo -e "\n${RED}ERROR: therrisd binary could not be created${NC}"
	exit -2
fi
...
sudo cp $SOURCES_PATH/bin/therrisd /usr/local/bin/
sudo strip /usr/local/bin/therrisd

# Backup data dir
if [ -d "$HOME/.Therris/" ]; then
	echo -e "\n${GREEN}Backing up ~/Therris (including wallet.dat)${NC}"
...
# Done
echo -e "\n${GREEN}Upgrade Complete - You can now run therrisd or fill out the config file in ~/Therris${NC}"
