...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Therris is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Therris is downloading blocks...");
...
Value getwork(const Array &params, bool fHelp) {
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Therris is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Therris is downloading blocks...");
