    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Therris and make it unusable. To
...
bool CDarksendPool::SignatureValid(const CScript& newSig, const CTxIn& newVin){
    ...
    if(found >= 0){ //might have to do this one input at a time?
        ...
        if (!VerifyScript(txNew.vin[n].scriptSig, sigPubKey, txNew, n, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRTHRXTENC, 0)){
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeTherrisAmount*COIN - pwalletMain->GetAnonymizedBalance();
