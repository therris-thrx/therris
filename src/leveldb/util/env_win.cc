...
BOOL Win32RandomAccessFile::_Init( LPCWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
        _hFile = ::CreateFileW(path,GENERTHRX_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,
...
BOOL Win32MapFile::_Init( LPCWSTR Path )
{
    ...
    _hFile = CreateFileW(Path,
                         GENERTHRX_READ | GENERTHRX_WRITE,
...
Status Win32Env::GetFileSize( const std::string& fname, uint64_t* file_size )
{
    ...
    HANDLE file = ::CreateFileW(wpath.c_str(),
        GENERTHRX_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
    LARGE_INTEGER li;
