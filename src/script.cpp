...
const char* GetOpName(opcodetype opcode)
{
    ...
    case OP_PTHRXK                   : return "OP_PTHRXK";
        ...
        while (pc < pend)
        {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKSIGVERIFY:
                {
                    ...
                    if ((flags & SCRIPT_VERIFY_STRTHRXTENC) && (!CheckSignatureEncoding(vchSig) || !CheckPubKeyEncoding(vchPubKey)))
                        return false;
...
bool static CheckSignatureEncoding(const valtype &vchSig, unsigned int flags, ScriptError* serror) {
    if ((flags & (SCRIPT_VERIFY_DERSIG | SCRIPT_VERIFY_LOW_S | SCRIPT_VERIFY_STRTHRXTENC)) != 0 && !IsDERSignature(vchSig)) {
        return set_error(serror, SCRIPT_ERR_SIG_DER);
    } else if ((flags & SCRIPT_VERIFY_LOW_S) != 0 && !IsLowDERSignature(vchSig, serror)) {
        // serror is set
        return false;
    } else if ((flags & SCRIPT_VERIFY_STRTHRXTENC) != 0 && !IsDefinedHashtypeSignature(vchSig)) {
...
bool static CheckPubKeyEncoding(const valtype &vchSig, unsigned int flags, ScriptError* serror) {
    if ((flags & SCRIPT_VERIFY_STRTHRXTENC) != 0 && !IsCompressedOrUncompressedPubKey(vchSig)) {
            ...
            switch (opcode)
            {
                ...
                case OP_CHECKMULTISIGVERIFY:
                {
                    ...
                    while (fSuccess && nSigsCount > 0)
                    {
                        ...
                        // distinguishable by CHECKMULTISIG NOT if the STRTHRXTENC flag is set.
