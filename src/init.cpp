...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Therris.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Therrisd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizetherrisamount=<n> " + _("Keep N Therris anonymized (default: 0)") + "\n";
...
bool AppInit2(boost::thread_group& threadGroup)
{
    ...
    _CrtSetReportFile(_CRT_WARN, CreateFileA("NUL", GENERTHRX_WRITE, 0, NULL, OPEN_EXISTING, 0, 0));
#endif
#if _MSC_VER >= 1400
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Therris is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Therris is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Therris version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Therris server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITTHRXAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Therris") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Therris to complete") << "\n";
    ...
    nAnonymizeTherrisAmount = GetArg("-anonymizetherrisamount", 0);
    if(nAnonymizeTherrisAmount > 999999) nAnonymizeTherrisAmount = 999999;
    if(nAnonymizeTherrisAmount < 2) nAnonymizeTherrisAmount = 2;
    ...
    LogPrintf("Anonymize Therris Amount %d\n", nAnonymizeTherrisAmount);
