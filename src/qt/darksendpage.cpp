...
void DarksendPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeTherrisAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTherrisAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        //strAnonymizeTherrisAmount = strAnonymizeTherrisAmount.remove(strAnonymizeTherrisAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTherrisAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeTherrisAmount*COIN) nMaxToAnonymize = nAnonymizeTherrisAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeTherrisAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeTherrisAmount));
        //strAnonymizeTherrisAmount = strAnonymizeTherrisAmount.remove(strAnonymizeTherrisAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTherrisAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
