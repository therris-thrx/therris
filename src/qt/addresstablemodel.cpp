...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            {
                wallet->SetAddressBookName(CTherriscoinAddress(strTemp).Get(), value.toString().toStdString());
            }
            break;
        case Address:
            ...
            if(CTherriscoinAddress(rec->address.toStdString()) == CTherriscoinAddress(value.toString().toStdString()))
            {
                editStatus = NO_CHANGES;
                return false;
            }
            ...
            else if(wallet->mapAddressBook.count(CTherriscoinAddress(value.toString().toStdString()).Get()))
            {
                editStatus = DUPLTHRXATE_ADDRESS;
                return false;
            }
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CTherriscoinAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CTherriscoinAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
            }
            break;
        }
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, int addressType)
{
    ...
    if(type == Send)
    {
        ...
        } else
        {
            ...
            {
                LOCK(wallet->cs_wallet);
                if (wallet->mapAddressBook.count(CTherriscoinAddress(strAddress).Get()))
                {
                    editStatus = DUPLTHRXATE_ADDRESS;
                    return QString();
                };
                
                wallet->SetAddressBookName(CTherriscoinAddress(strAddress).Get(), strLabel);
            }
        }
    }
    else if(type == Receive)
    {
        ...
        } else
        {
            ...
            strAddress = CTherriscoinAddress(newKey.GetID()).ToString();
...
{
    {
        ...
        {
            CTherriscoinAddress address_parsed(sAddr);
