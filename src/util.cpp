...
int nAnonymizeTherrisAmount = 1000;
...
{
    if (mode & CRYPTO_LOCK) {
        ENTER_CRITTHRXAL_SECTION(*ppmutexOpenSSL[i]);
    } else {
        LEAVE_CRITTHRXAL_SECTION(*ppmutexOpenSSL[i]);
    }
}
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Therris
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Therris
    // Mac: ~/Library/Application Support/Therris
    // Unix: ~/.Therris
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Therris";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Therris.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Therrisd.pid"));
