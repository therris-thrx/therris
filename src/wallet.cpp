...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CTherriscoinAddress(redeemScript.GetID()).ToString();
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-Therris-address
                    CScript scriptChange;
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        if (fDebug)
        {
            ...
            CTherriscoinAddress coinAddress(keyID);
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    CTherriscoinAddress addrTo(ckidTo);
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key, unsigned int* nNonceBlock)
{
    ...
    if (pindexPrev->nHeight+1 < GetForkHeightOne())
    {
        ...
        if(hasPayment && payeerewardpercent > 0 && payeerewardpercent < 100){
            ...
            CTherriscoinAddress address4(address3);
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CTherriscoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CTherriscoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CTherriscoinAddress(address).ToString());
}
...
bool CWallet::ImportPrivateKey(CTherriscoinSecret vchSecret, string strLabel, bool fRescan) {
    if (fWalletUnlockStakingOnly)
        return false;
